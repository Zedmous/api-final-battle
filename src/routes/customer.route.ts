...
const userValidator = new CustomerValidator();
const router = Router();
...
router.post("/",userValidator.validateCustomer,validateFields, userController.create);//http://localhost:3800/api/customers
router.put("/:id",userValidator.validateCustomer,validateFields, userController.update);//http://localhost:3800/api/customers/1
